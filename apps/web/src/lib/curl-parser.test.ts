...
describe("parseCurlCommand", () => {
	...
	it("extracts JSON body via --data-raw double-quoted", () => {
		const r = parseCurlCommand(`curl https://x.test --data-raw "{\\"a\\":1}"`);
...
describe("detectApiType", () => {
	it("detects images by URL", () => {
		expect(detectApiType("https://x/v1/images/generations", null)).toBe(
			"images",
		);
	});
	...
	it("detects rerank by body when URL is generic", () => {
		expect(detectApiType("https://x/foo", { query: "q", texts: ["a"] })).toBe(
			"rerank",
		);
	});
	...
	it("detects embeddings by body", () => {
		expect(detectApiType("https://x/foo", { input: "hello" })).toBe(
			"embeddings",
		);
	});
