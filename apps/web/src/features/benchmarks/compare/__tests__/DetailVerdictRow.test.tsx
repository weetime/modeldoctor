...
function makeBenchmark(id: string, p95: number): Benchmark {
  return {
    id,
    userId: null,
    connectionId: null,
...
describe("DetailVerdictRow", () => {
  ...
  it("renders 3 verdict badges when baseline benchmark loads", async () => {
    const current = makeBenchmark("c", 240);
    // Sequence: useBaselines list, useBenchmarkDetail for baseline benchmark
    vi.mocked(api.get)
      ...
      .mockResolvedValueOnce(makeBenchmark("br", 200));

    render(<DetailVerdictRow benchmark={current} baselineId="b_1" />, { wrapper: Wrapper });
