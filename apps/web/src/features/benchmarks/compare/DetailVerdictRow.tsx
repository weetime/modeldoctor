...
export function DetailVerdictRow({ benchmark, baselineId }: DetailVerdictRowProps) {
  ...
  if (baselineQuery.isLoading || (baselineBenchmarkId.length > 0 && baselineBenchmark.isLoading)) {
    return <div className="text-xs text-muted-foreground">{t("detail.verdict.loading")}</div>;
  }

  if (baselineQuery.isError || baselineBenchmark.isError) {
    return (
  ...
  const baseline = baselineBenchmark.data;
