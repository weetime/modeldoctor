...
const base = {
  ...
  perDomain: {
    airline: { pass1: 0.25, passK: 0.2, tasks: 20 },
    retail: { pass1: 0.5, passK: 0.4, tasks: 20 },
  },
  attribution: {},
  highlights: { successSimId: null, successDomain: null, failureSimId: null, failureDomain: null },
} as const;
...
describe("computeGate", () => {
  ...
  it("perDomainFloor FAILS when a domain is below its floor", () => {
    const g = computeGate(
      base as any,
      { mode: "perDomainFloor", perDomainFloor: { airline: 0.3 } },
      null,
    );
  ...
  it("perDomainFloor PASSES when all domains meet floor", () => {
    const g = computeGate(
      base as any,
      { mode: "perDomainFloor", perDomainFloor: { airline: 0.2, retail: 0.4 } },
      null,
    );
  ...
  it("baselineRegression FAILED when drop exceeds pp", () => {
    const g = computeGate(
      base as any,
      { mode: "baselineRegression", baselineRegressionPp: 5 },
      0.5,
    );
  ...
  it("baselineRegression WARNING within half the threshold", () => {
    const g = computeGate(
      base as any,
      { mode: "baselineRegression", baselineRegressionPp: 16 },
      0.5,
    );
  ...
  it("baselineRegression PASSED when no meaningful drop", () => {
    const g = computeGate(
      base as any,
      { mode: "baselineRegression", baselineRegressionPp: 30 },
      0.41,
    );
    expect(g.result).toBe("PASSED");
  });
