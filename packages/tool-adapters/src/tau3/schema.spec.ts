...
import { tau3ParamsSchema, tau3ReportSchema, tau3ParamDefaults } from "./schema.js";
...
describe("tau3ParamsSchema", () => {
  ...
  it("accepts numTasksPerDomain null (full set)", () => {
    expect(tau3ParamsSchema.parse({ ...tau3ParamDefaults, numTasksPerDomain: null }).numTasksPerDomain).toBeNull();
  });
  ...
  it("perDomainFloor gate parses", () => {
    const p = tau3ParamsSchema.parse({ ...tau3ParamDefaults,
...
describe("tau3ReportSchema", () => {
  it("parses a summary.json shape", () => {
    const r = tau3ReportSchema.parse({
      kind: "agent-tau3", userSimModel: "deepseek-v3", numTrials: 3,
