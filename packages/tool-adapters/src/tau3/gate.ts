import type { Tau3Params, Tau3Report } from "./schema.js";

export type GateResult = {
  mode: string;
  result: "PASSED" | "WARNING" | "FAILED" | null;
  detail?: string;
};
...
export function computeGate(
  ...
  if (dropPp >= th)
    return { mode: gate.mode, result: "FAILED", detail: `-${dropPp.toFixed(1)}pp vs baseline` };
  if (dropPp >= th / 2)
    return { mode: gate.mode, result: "WARNING", detail: `-${dropPp.toFixed(1)}pp vs baseline` };
  return {
    mode: gate.mode,
    result: "PASSED",
    detail: `${dropPp <= 0 ? "+" : "-"}${Math.abs(dropPp).toFixed(1)}pp vs baseline`,
  };
}
